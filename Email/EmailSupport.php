...
class EmailSupport
{
    ...
    public function supportEmailBody($first_name, $last_name, $email, $subject, $message)
    {
        $supportEmailBodyTemplate = ORB_PRODUCTS_SERVICES . 'Templates/TemplatesEmailBodySupport.php';
        ...
        if (file_exists($supportEmailBodyTemplate)) {
            $body = file_get_contents($supportEmailBodyTemplate);
            ...
            throw new Exception('Unable to locate support email template.');
        }
    }

    public function sendSupportEmail($first_name, $last_name, $email, $subject, $message)
    {
        try {
            $this->mailer->isSMTP();
            ...
            $this->mailer->Body = $this->supportEmailBody($first_name, $last_name, $email, $subject, $message);
